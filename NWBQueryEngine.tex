...
Despite the fact the NWB Query engine can be easily deployed and used by being aware the usage of a tool can be still obstacle for some users. It is because lot of user working in laboratories do not have system administrators who could be helpful with the deployment of such a tool. Moreover current trends go toward to cloud based solutions installed on remote servers accessible via thin clients from users desktops. A concept of shifting neurodata laboratories from locally maintained systems to a cloud based solutions are discussed e. g \citep{ROSENTHAL2010342}. 

Following this trend and to to make working with the NWB Query engine even much simpler we provided a web based system that allows execute queries on the NWB files comfortably from a web browser. Another motivation for us have been an assumption that data in laboratories are stored on a central data storage and not distributed over individual working stations. We have supposed that such a web interface facilitates searching within data also for not experienced users.

The web interface preview is in Figure \ref{web_interface}. It is a simple and clear page with only a basic information and a search box for the user input query. When the user run a query it is executed on background. Because it can be a time consuming task when lot of data is stored the web page is implemented dynamically. It concretely means that data are read and displayed in sequence once they are loaded. Moreover a progress bar informs the user how many data is browsed already. Once a required file is found the user can download it.

From the technological point of view the Wen Interface is implemented in a Spring framework. It is a implementation of Dependency Injection design pattern that allows, besides, easily integrates various modules and it provides integration with other used technologies. Namely Wicket, used for the user layer, is an Ajax-based framework used for dynamic loading of data. Next, Apache Bootstrap is a library of CSS templates that facilitated designing of the web page.
