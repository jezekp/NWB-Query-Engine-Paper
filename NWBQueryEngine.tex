...
Because specific requiremensts defined in Section \ref{Scope_and_requirements} a modular architecture has been designed. The NWB query engine contains three main components. The first component is a Query parser that takes a user query and parses it according to a grammar, described later in Section \ref{Query_Grammar}, and generates a subexpressions tree. Such expressions are managed by a next block NWB Processor. This processor iterates over a list of expressions that are executed on a NWB file implemented in HDF5 using a HDF5 Connector implemented in a third block File API. Data obtained from a NWB file are returned back to the NWB Processor wrapped in a NWB result. 
